...
//		delete_index(doc_id);
	...
	private static void create_index_type_mapping() throws IOException{
		...
		XContentBuilder builder_mapping = XContentFactory.jsonBuilder()
				.startObject().startObject(type_name).startObject("properties")
					.startObject("CompanyCode").field("type", "integer").endObject()//还可以指定其他属性：store,indexAnalyzer,searchAnalyzer等等
...
//		ActionFuture<IndexResponse> response = client.index(Requests.indexRequest(index_alias).id("3").type(type_name).source(map));
	...
	private static void bulk_update_index() throws ParseException, InterruptedException, ExecutionException {
		...
		request.add(Requests.indexRequest(index_alias).type(type_name).id("AVw1aJ-Dnfi1GLkkfizU").source(map));
		
		map = Maps.newHashMap();
		map.put("ChiSpelling", "SZGT_UPDATE");
		map.put("LastClose", 11.00);
		request.add(Requests.indexRequest(index_alias).type(type_name).id("AVw1aJ-Dnfi1GLkkfizW").source(map));
